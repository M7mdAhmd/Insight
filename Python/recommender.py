...
def add_review_to_db(conn, user_id, paper_id, rating):
    ...
    try:
        ...
        if existing:
            ...
            cursor.execute("SELECT ISNULL(MAX(Review_ID), 0) + 1 FROM Review")
            new_review_id = cursor.fetchone()[0]
            
            cursor.execute("""
                INSERT INTO Review (Review_ID, Rating, Review_Date, Paper_ID, Researcher_ID)
                VALUES (?, ?, ?, ?, ?)
            """, (new_review_id, rating, review_date, paper_id, user_id))
...
def add_download_to_db(conn, user_id, paper_id):
    ...
    try:
        ...
        cursor.execute("SELECT ISNULL(MAX(Download_ID), 0) + 1 FROM Download")
        new_download_id = cursor.fetchone()[0]
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (new_download_id, download_date, paper_id, user_id))
...
def add_search_to_db(conn, user_id, query):
    ...
    try:
        cursor.execute("SELECT ISNULL(MAX(Search_ID), 0) + 1 FROM Search")
        new_search_id = cursor.fetchone()[0]
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (new_search_id, query, search_date, user_id))
